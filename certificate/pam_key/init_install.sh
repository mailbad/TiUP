...
school_name="ruc test bjtiup tiup "
...
url_domain_name="static uplod 404 edu-storage imserver open pic-img staticjs rpacl search tcs uc api docker-reg"
...
demo_key_init () {
	...
	find ./ -type f -name "*.crt" -mtime -80 |sed 's/.crt//g'|grep "/${domain_name}$/" && exit 0
	...
	#echo ${school_name} ${url_domain_name} |sed "s/ /.${domain_name} /g"|sed "s/$/.${domain_name}/g" |sed "s/^/222.29.240.58222.29.240.58     ${domain_name} /g" >> /etc/hosts
	./letsencrypt.sh letsencrypt.conf || echo rm -rf ${domain_name%%.*}.*  *.chained.crt *.csr *.pem
	cp -arf hosts /etc/hosts
